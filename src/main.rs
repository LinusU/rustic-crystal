...
fn main() -> Result<(), &'static str> {
    ...
    let cpal_audio_stream = match CpalPlayer::get() {
        Some((v, s)) => {
            cpu.mmu.sound = Some(Sound::new_cgb(Box::new(v) as Box<dyn rustic_crystal::AudioPlayer>));
            s
        },
        None => {
...
fn glutin_to_keypad(key: glium::glutin::event::VirtualKeyCode) -> Option<rustic_crystal::KeypadKey> {
    use glium::glutin::event::VirtualKeyCode;
    match key {
            ...
            match receiver.try_recv() {
                Ok(event) => {
                    match event {
                        ...
                        GBEvent::SpeedDown => { limit_speed = true; cpu.mmu.sound.as_mut().unwrap().sync(); }
                    }
                },
...
fn set_window_size(window: &glium::glutin::window::Window, scale: u32) {
    ...
    let physical_size = PhysicalSize::<u32>::from((rustic_crystal::SCREEN_W as u32 * scale, rustic_crystal::SCREEN_H as u32 * scale));
...
fn cpal_thread<T: cpal::Sample + cpal::FromSample<f32>>(outbuffer: &mut[T], audio_buffer: &Arc<Mutex<Vec<(f32, f32)>>>) {
