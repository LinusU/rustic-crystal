...
impl GpuAtlas {
    pub const fn new(src: &'static [u8], width: usize) -> GpuAtlas {
        GpuAtlas { src, width }
    }
}
...
impl Gpu {
    pub fn new_cgb(update_screen: SyncSender<Vec<u8>>) -> Gpu {
        Gpu {
            mode: 0,
            modeclock: 0,
            line: 0,
        ...
        for layer in &self.layers {
            for (idx, tile) in layer.background.iter().enumerate() {
                if let Some(tile) = tile {
                    ...
                    for dy in 0..32 {
                        for dx in 0..32 {
                            let src_idx = ((((tile.src_y * 32) + dy) * tile.atlas.width)
                                + (tile.src_x * 32)
                                + dx)
                                * 4;
                            let dst_idx = (((dst_y + dy) * SCREEN_W) + dst_x + dx) * 3;

                            let alpha = tile.atlas.src[src_idx + 3] as f32 / 255.0;
                            let inv_alpha = 1.0 - alpha;

                            screen[dst_idx + 0] = (screen[dst_idx + 0] as f32 * inv_alpha
                                + tile.atlas.src[src_idx + 0] as f32 * alpha)
                                as u8;
                            screen[dst_idx + 1] = (screen[dst_idx + 1] as f32 * inv_alpha
                                + tile.atlas.src[src_idx + 1] as f32 * alpha)
                                as u8;
                            screen[dst_idx + 2] = (screen[dst_idx + 2] as f32 * inv_alpha
                                + tile.atlas.src[src_idx + 2] as f32 * alpha)
                                as u8;
                        }
                    }
                }
    ...
    fn setcolor(&mut self, x: usize, color: u8) {
        self.data[self.line as usize * GB_SCREEN_W * 3 + x * 3] = color;
