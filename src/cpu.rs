...
impl<'a> Cpu<'a> {
    ...
    fn step(&mut self) -> u32 {
        ...
        match opcode {
            ...
            0x36 => { let v = self.fetch_byte(); self.mmu.wb(self.reg.hl(), v); 3 },
            0x37 => { self.reg.set_flag(C, true); self.reg.set_flag(H, false); self.reg.set_flag(N, false); 1 },
            0x38 => { if self.reg.flag(C) { self.cpu_jr(); 3 } else { self.reg.pc += 1; 2  } },
            ...
            0x3F => { let v = !self.reg.flag(C); self.reg.set_flag(C, v); self.reg.set_flag(H, false); self.reg.set_flag(N, false); 1 },
            0x40 => { 1 },
            ...
            0xC0 => { if !self.reg.flag(Z) { self.reg.pc = self.stack_pop(); 5 } else { 2 } },
            0xC1 => { let v = self.stack_pop(); self.reg.set_bc(v); 3 },
            0xC2 => { if !self.reg.flag(Z) { self.reg.pc = self.fetch_word(); 4 } else { self.reg.pc += 2; 3 } },
            0xC3 => { self.reg.pc = self.fetch_word(); 4 },
            0xC4 => { if !self.reg.flag(Z) { self.stack_push(self.reg.pc + 2); self.reg.pc = self.fetch_word(); 6 } else { self.reg.pc += 2; 3 } },
            0xC5 => { self.stack_push(self.reg.bc()); 4 },
            0xC6 => { let v = self.fetch_byte(); self.alu_add(v, false); 2 },
            0xC7 => { self.stack_push(self.reg.pc); self.reg.pc = 0x00; 4 },
            0xC8 => { if self.reg.flag(Z) { self.reg.pc = self.stack_pop(); 5 } else { 2 } },
            0xC9 => { self.reg.pc = self.stack_pop(); 4 },
            0xCA => { if self.reg.flag(Z) { self.reg.pc = self.fetch_word(); 4 } else { self.reg.pc += 2; 3 } },
            0xCB => { self.step_cb() },
            0xCC => { if self.reg.flag(Z) { self.stack_push(self.reg.pc + 2); self.reg.pc = self.fetch_word(); 6 } else { self.reg.pc += 2; 3 } },
            0xCD => { self.stack_push(self.reg.pc + 2); self.reg.pc = self.fetch_word(); 6 },
            0xCE => { let v = self.fetch_byte(); self.alu_add(v, true); 2 },
            0xCF => { self.stack_push(self.reg.pc); self.reg.pc = 0x08; 4 },
            0xD0 => { if !self.reg.flag(C) { self.reg.pc = self.stack_pop(); 5 } else { 2 } },
            0xD1 => { let v = self.stack_pop(); self.reg.set_de(v); 3 },
            0xD2 => { if !self.reg.flag(C) { self.reg.pc = self.fetch_word(); 4 } else { self.reg.pc += 2; 3 } },
            0xD4 => { if !self.reg.flag(C) { self.stack_push(self.reg.pc + 2); self.reg.pc = self.fetch_word(); 6 } else { self.reg.pc += 2; 3 } },
            0xD5 => { self.stack_push(self.reg.de()); 4 },
            0xD6 => { let v = self.fetch_byte(); self.alu_sub(v, false); 2 },
            0xD7 => { self.stack_push(self.reg.pc); self.reg.pc = 0x10; 4 },
            0xD8 => { if self.reg.flag(C) { self.reg.pc = self.stack_pop(); 5 } else { 2 } },
            0xD9 => { self.reg.pc = self.stack_pop(); self.setei = 1; 4 },
            0xDA => { if self.reg.flag(C) { self.reg.pc = self.fetch_word(); 4 } else { self.reg.pc += 2; 3 } },
            0xDC => { if self.reg.flag(C) { self.stack_push(self.reg.pc + 2); self.reg.pc = self.fetch_word(); 6 } else { self.reg.pc += 2; 3 } },
            0xDE => { let v = self.fetch_byte(); self.alu_sub(v, true); 2 },
            0xDF => { self.stack_push(self.reg.pc); self.reg.pc = 0x18; 4 },
            0xE0 => { let a = 0xFF00 | self.fetch_byte() as u16; self.mmu.wb(a, self.reg.a); 3 },
            0xE1 => { let v = self.stack_pop(); self.reg.set_hl(v); 3 },
            0xE2 => { self.mmu.wb(0xFF00 | self.reg.c as u16, self.reg.a); 2 },
            0xE5 => { self.stack_push(self.reg.hl()); 4 },
            0xE6 => { let v = self.fetch_byte(); self.alu_and(v); 2 },
            0xE7 => { self.stack_push(self.reg.pc); self.reg.pc = 0x20; 4 },
            0xE8 => { self.reg.sp = self.alu_add16imm(self.reg.sp); 4 },
            0xE9 => { self.reg.pc = self.reg.hl(); 1 },
            0xEA => { let a = self.fetch_word(); self.mmu.wb(a, self.reg.a); 4 },
            0xEE => { let v = self.fetch_byte(); self.alu_xor(v); 2 },
            0xEF => { self.stack_push(self.reg.pc); self.reg.pc = 0x28; 4 },
            0xF0 => { let a = 0xFF00 | self.fetch_byte() as u16; self.reg.a = self.mmu.rb(a); 3 },
            0xF1 => { let v = self.stack_pop() & 0xFFF0; self.reg.set_af(v); 3 },
            0xF2 => { self.reg.a = self.mmu.rb(0xFF00 | self.reg.c as u16); 2 },
            0xF3 => { self.setdi = 2; 1 },
            0xF5 => { self.stack_push(self.reg.af()); 4 },
            0xF6 => { let v = self.fetch_byte(); self.alu_or(v); 2 },
            0xF7 => { self.stack_push(self.reg.pc); self.reg.pc = 0x30; 4 },
            0xF8 => { let r = self.alu_add16imm(self.reg.sp); self.reg.set_hl(r); 3 },
            0xF9 => { self.reg.sp = self.reg.hl(); 2 },
            0xFA => { let a = self.fetch_word(); self.reg.a = self.mmu.rb(a); 4 },
            0xFB => { self.setei = 2; 1 },
            0xFE => { let v = self.fetch_byte(); self.alu_cp(v); 2 },
            0xFF => { self.stack_push(self.reg.pc); self.reg.pc = 0x38; 4 },
    ...
    fn step_cb(&mut self) -> u32 {
        ...
        match opcode {
            ...
            0x80 => { self.reg.b &= !(1 << 0); 2 },
            0x81 => { self.reg.c &= !(1 << 0); 2 },
            0x82 => { self.reg.d &= !(1 << 0); 2 },
            0x83 => { self.reg.e &= !(1 << 0); 2 },
            0x84 => { self.reg.h &= !(1 << 0); 2 },
            0x85 => { self.reg.l &= !(1 << 0); 2 },
            0x86 => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 0); self.mmu.wb(a, v); 4 },
            0x87 => { self.reg.a &= !(1 << 0); 2 },
            0x88 => { self.reg.b &= !(1 << 1); 2 },
            0x89 => { self.reg.c &= !(1 << 1); 2 },
            0x8A => { self.reg.d &= !(1 << 1); 2 },
            0x8B => { self.reg.e &= !(1 << 1); 2 },
            0x8C => { self.reg.h &= !(1 << 1); 2 },
            0x8D => { self.reg.l &= !(1 << 1); 2 },
            0x8E => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 1); self.mmu.wb(a, v); 4 },
            0x8F => { self.reg.a &= !(1 << 1); 2 },
            0x90 => { self.reg.b &= !(1 << 2); 2 },
            0x91 => { self.reg.c &= !(1 << 2); 2 },
            0x92 => { self.reg.d &= !(1 << 2); 2 },
            0x93 => { self.reg.e &= !(1 << 2); 2 },
            0x94 => { self.reg.h &= !(1 << 2); 2 },
            0x95 => { self.reg.l &= !(1 << 2); 2 },
            0x96 => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 2); self.mmu.wb(a, v); 4 },
            0x97 => { self.reg.a &= !(1 << 2); 2 },
            0x98 => { self.reg.b &= !(1 << 3); 2 },
            0x99 => { self.reg.c &= !(1 << 3); 2 },
            0x9A => { self.reg.d &= !(1 << 3); 2 },
            0x9B => { self.reg.e &= !(1 << 3); 2 },
            0x9C => { self.reg.h &= !(1 << 3); 2 },
            0x9D => { self.reg.l &= !(1 << 3); 2 },
            0x9E => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 3); self.mmu.wb(a, v); 4 },
            0x9F => { self.reg.a &= !(1 << 3); 2 },
            0xA0 => { self.reg.b &= !(1 << 4); 2 },
            0xA1 => { self.reg.c &= !(1 << 4); 2 },
            0xA2 => { self.reg.d &= !(1 << 4); 2 },
            0xA3 => { self.reg.e &= !(1 << 4); 2 },
            0xA4 => { self.reg.h &= !(1 << 4); 2 },
            0xA5 => { self.reg.l &= !(1 << 4); 2 },
            0xA6 => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 4); self.mmu.wb(a, v); 4 },
            0xA7 => { self.reg.a &= !(1 << 4); 2 },
            0xA8 => { self.reg.b &= !(1 << 5); 2 },
            0xA9 => { self.reg.c &= !(1 << 5); 2 },
            0xAA => { self.reg.d &= !(1 << 5); 2 },
            0xAB => { self.reg.e &= !(1 << 5); 2 },
            0xAC => { self.reg.h &= !(1 << 5); 2 },
            0xAD => { self.reg.l &= !(1 << 5); 2 },
            0xAE => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 5); self.mmu.wb(a, v); 4 },
            0xAF => { self.reg.a &= !(1 << 5); 2 },
            0xB0 => { self.reg.b &= !(1 << 6); 2 },
            0xB1 => { self.reg.c &= !(1 << 6); 2 },
            0xB2 => { self.reg.d &= !(1 << 6); 2 },
            0xB3 => { self.reg.e &= !(1 << 6); 2 },
            0xB4 => { self.reg.h &= !(1 << 6); 2 },
            0xB5 => { self.reg.l &= !(1 << 6); 2 },
            0xB6 => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 6); self.mmu.wb(a, v); 4 },
            0xB7 => { self.reg.a &= !(1 << 6); 2 },
            0xB8 => { self.reg.b &= !(1 << 7); 2 },
            0xB9 => { self.reg.c &= !(1 << 7); 2 },
            0xBA => { self.reg.d &= !(1 << 7); 2 },
            0xBB => { self.reg.e &= !(1 << 7); 2 },
            0xBC => { self.reg.h &= !(1 << 7); 2 },
            0xBD => { self.reg.l &= !(1 << 7); 2 },
            0xBE => { let a = self.reg.hl(); let v = self.mmu.rb(a) & !(1 << 7); self.mmu.wb(a, v); 4 },
            0xBF => { self.reg.a &= !(1 << 7); 2 },
            0xC0 => { self.reg.b |= 1 << 0; 2 },
            0xC1 => { self.reg.c |= 1 << 0; 2 },
            0xC2 => { self.reg.d |= 1 << 0; 2 },
            0xC3 => { self.reg.e |= 1 << 0; 2 },
            0xC4 => { self.reg.h |= 1 << 0; 2 },
            0xC5 => { self.reg.l |= 1 << 0; 2 },
            0xC6 => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 0); self.mmu.wb(a, v); 4 },
            0xC7 => { self.reg.a |= 1 << 0; 2 },
            0xC8 => { self.reg.b |= 1 << 1; 2 },
            0xC9 => { self.reg.c |= 1 << 1; 2 },
            0xCA => { self.reg.d |= 1 << 1; 2 },
            0xCB => { self.reg.e |= 1 << 1; 2 },
            0xCC => { self.reg.h |= 1 << 1; 2 },
            0xCD => { self.reg.l |= 1 << 1; 2 },
            0xCE => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 1); self.mmu.wb(a, v); 4 },
            0xCF => { self.reg.a |= 1 << 1; 2 },
            0xD0 => { self.reg.b |= 1 << 2; 2 },
            0xD1 => { self.reg.c |= 1 << 2; 2 },
            0xD2 => { self.reg.d |= 1 << 2; 2 },
            0xD3 => { self.reg.e |= 1 << 2; 2 },
            0xD4 => { self.reg.h |= 1 << 2; 2 },
            0xD5 => { self.reg.l |= 1 << 2; 2 },
            0xD6 => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 2); self.mmu.wb(a, v); 4 },
            0xD7 => { self.reg.a |= 1 << 2; 2 },
            0xD8 => { self.reg.b |= 1 << 3; 2 },
            0xD9 => { self.reg.c |= 1 << 3; 2 },
            0xDA => { self.reg.d |= 1 << 3; 2 },
            0xDB => { self.reg.e |= 1 << 3; 2 },
            0xDC => { self.reg.h |= 1 << 3; 2 },
            0xDD => { self.reg.l |= 1 << 3; 2 },
            0xDE => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 3); self.mmu.wb(a, v); 4 },
            0xDF => { self.reg.a |= 1 << 3; 2 },
            0xE0 => { self.reg.b |= 1 << 4; 2 },
            0xE1 => { self.reg.c |= 1 << 4; 2 },
            0xE2 => { self.reg.d |= 1 << 4; 2 },
            0xE3 => { self.reg.e |= 1 << 4; 2 },
            0xE4 => { self.reg.h |= 1 << 4; 2 },
            0xE5 => { self.reg.l |= 1 << 4; 2 },
            0xE6 => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 4); self.mmu.wb(a, v); 4 },
            0xE7 => { self.reg.a |= 1 << 4; 2 },
            0xE8 => { self.reg.b |= 1 << 5; 2 },
            0xE9 => { self.reg.c |= 1 << 5; 2 },
            0xEA => { self.reg.d |= 1 << 5; 2 },
            0xEB => { self.reg.e |= 1 << 5; 2 },
            0xEC => { self.reg.h |= 1 << 5; 2 },
            0xED => { self.reg.l |= 1 << 5; 2 },
            0xEE => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 5); self.mmu.wb(a, v); 4 },
            0xEF => { self.reg.a |= 1 << 5; 2 },
            0xF0 => { self.reg.b |= 1 << 6; 2 },
            0xF1 => { self.reg.c |= 1 << 6; 2 },
            0xF2 => { self.reg.d |= 1 << 6; 2 },
            0xF3 => { self.reg.e |= 1 << 6; 2 },
            0xF4 => { self.reg.h |= 1 << 6; 2 },
            0xF5 => { self.reg.l |= 1 << 6; 2 },
            0xF6 => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 6); self.mmu.wb(a, v); 4 },
            0xF7 => { self.reg.a |= 1 << 6; 2 },
            0xF8 => { self.reg.b |= 1 << 7; 2 },
            0xF9 => { self.reg.c |= 1 << 7; 2 },
            0xFA => { self.reg.d |= 1 << 7; 2 },
            0xFB => { self.reg.e |= 1 << 7; 2 },
            0xFC => { self.reg.h |= 1 << 7; 2 },
            0xFD => { self.reg.l |= 1 << 7; 2 },
            0xFE => { let a = self.reg.hl(); let v = self.mmu.rb(a) | (1 << 7); self.mmu.wb(a, v); 4 },
            0xFF => { self.reg.a |= 1 << 7; 2 },
        }
    }

    fn alu_add(&mut self, b: u8, usec: bool) {
        let c = if usec && self.reg.flag(C) { 1 } else { 0 };
        let a = self.reg.a;
    ...
    fn alu_inc(&mut self, a: u8) -> u8 {
        ...
        self.reg.set_flag(Z, r == 0);
        self.reg.set_flag(H, (a & 0x0F) + 1 > 0x0F);
        self.reg.set_flag(N, false);
        r
    }
    ...
    fn alu_add16(&mut self, b: u16) {
        ...
        self.reg.set_flag(H, (a & 0x07FF) + (b & 0x07FF) > 0x07FF);
        self.reg.set_flag(N, false);
        self.reg.set_flag(C, a > 0xFFFF - b);
        self.reg.set_hl(r);
    }
    ...
    fn alu_rl(&mut self, a: u8) -> u8 {
        let c = a & 0x80 == 0x80;
        let r = (a << 1) | (if self.reg.flag(C) { 1 } else { 0 });
        self.alu_srflagupdate(r, c);
        r
    }
    ...
    fn alu_rr(&mut self, a: u8) -> u8 {
        let c = a & 0x01 == 0x01;
        let r = (a >> 1) | (if self.reg.flag(C) { 0x80 } else { 0 });
        self.alu_srflagupdate(r, c);
        r
    }
    ...
    fn alu_daa(&mut self) {
        ...
        let mut adjust = if self.reg.flag(C) { 0x60 } else { 0x00 };
        if self.reg.flag(H) {
            adjust |= 0x06;
        };
        if !self.reg.flag(N) {
